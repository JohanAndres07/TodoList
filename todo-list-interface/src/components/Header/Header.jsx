    ...
    return (
        <header className='w-full h-[8rem] md-2 md:border-b-[.016rem] md:border-gray-400 md:mb-10 md:flex md:justify-between md:h-[4rem] lg:justify-around'>
            ...
            <nav className='flex h-[4rem] justify-center item-center p-4'>
                <input className={`mx-4 pb-2 cursor-pointer font-Jura text-[1.5rem] text-blue-normal ${currentView === 'task' ? 'font-bold border-b-4 border-blue-normal' : ''}`} onClick={()=> toggleView('task')} type="button" value="Task" />
                <input className={`mx-4 pb-2 cursor-pointer font-Jura text-[1.5rem] text-blue-normal ${currentView !== 'task' ? 'font-bold border-b-4 border-blue-normal': ''}`} onClick={()=> toggleView('group')} type="button" value="Group" />
            </nav>
        </header>
    )
}
